...
let mkcase env sigma c ty constrs =
  ...
  let _len = Context.Rel.length ctx in
  ...
  let brs = 
    Array.map2_i (fun i id (ctx, cty) ->
      ...
      let realargs, pars = Context.Rel.chop (Context.Rel.length args - params) args in
...
let mk_eq env env' evd args args' =
  ...
  let _, _, make' = Sigma_types.telescope env' evd args' in
  let make = lift (Context.Rel.length args + 1) make in
...
let derive_no_confusion ~pm env sigma0 ~poly (ind,u as indu) =
  ...
  let argr, argty, x, ctx, argsctx =
    if Array.length rest = 0 then
      ERelevance.make oneind.mind_relevance, mkApp (mkIndU indu, argsvect), mkRel 1, ctx, Context.Rel.empty
    else
      ...
      let () = evd := evm in
      let _, pred' = Term.decompose_lambda_n (Context.Rel.length pars) (EConstr.to_constr !evd pred) in
      let indty = mkApp (sigma, [|idx; of_constr pred'|]) in
      (* sigma is not sort poly (at least for now) *)
      ERelevance.relevant, nf_betaiotazeta env !evd indty, mkProj (Lazy.force coq_pr2, ERelevance.relevant, mkRel 1), pars, (Context.Rel.firstn lenargs ctx)
  in
  ...
  in
  let lenindices = Context.Rel.length argsctx in
  let pred =
    let elim =
      ...
      let app = pack_ind_with_parlift (args + 2) in
          ...
          (Context.Rel.add (of_tuple (nameR xid, None, ind_with_parlift (lenindices + 1)))
             (lift_rel_context 1 argsctx))
    in
      mkcase env !evd x elim (fun ind i id nparams args arity ->
        let ydecl = (nameR yid, None, pack_ind_with_parlift (Context.Rel.length args + 1)) in
        let env' = push_rel_context (Context.Rel.add (of_tuple ydecl) args) env in
        let argsctx = lift_rel_context (Context.Rel.length args + 2) argsctx in
        let elimdecl = (nameR yid, None, ind_with_parlift (Context.Rel.length args + lenindices + 2)) in
	        mkLambda_or_LetIn (of_tuple ydecl)
            (mkcase env' !evd x
	        (it_mkLambda_or_LetIn s (Context.Rel.add (of_tuple elimdecl) argsctx))
	        (fun _ i' id' nparams args' arity' ->
	          if i = i' then
	            if Context.Rel.length args = 0 then tru
